...
function Test-ItemIsScript {
    ...
    # Check if it's the script file itself
    if ($ItemPath -eq $script:ScriptPath) {
        return $true
    }
    
    # Check if it's in the same directory as the script and is a PowerShell file
    $itemDirectory = Split-Path -Parent $ItemPath
    $itemName = Split-Path -Leaf $ItemPath
    
    if ($itemDirectory -eq $script:ScriptDirectory -and $itemName.EndsWith('.ps1')) {
        return $true
    }
...
# Helper function for safe item removal with external drive and script protection
function Remove-SafeItemWithExternalProtection {
    ...
    # Check if item is on external drive
    if (Test-ItemOnExternalDrive $ItemPath) {
        Write-Host "Preserved (external drive): $ItemPath" -ForegroundColor Blue
        return
    }
    
    # Check if item is the script itself or related PowerShell files
    if (Test-ItemIsScript $ItemPath) {
        Write-Host "Preserved (script file): $ItemPath" -ForegroundColor Cyan
        return
    }
...
function Remove-ThirdPartyBloatware {
    ...
    Write-Host "NOTE: Google Chrome is preserved and will not be removed" -ForegroundColor Green
...
# Generic function to clean folder with external drive protection
    ...
    if (Test-Path $FolderPath) {
        try {
            ...
            foreach ($item in $items) {
                Remove-SafeItemWithExternalProtection -ItemPath $item.FullName -ItemName $item.Name
            }
        }
        catch {
...
# Function to clean downloads folder (excluding external drives)
function Clear-DownloadsFolder {
    Clear-FolderWithExternalProtection -FolderPath "$env:USERPROFILE\Downloads" -FolderName "Downloads Folder"
}

# Function to clear desktop (excluding external drives)
function Clear-Desktop {
    Clear-FolderWithExternalProtection -FolderPath "$env:USERPROFILE\Desktop" -FolderName "Desktop"
}
...
function Main {
    ...
    # Display script protection information
    Write-Host "`nScript Protection:" -ForegroundColor Green
    Write-Host "  - Script location: $script:ScriptPath" -ForegroundColor Green
    Write-Host "  - Google Chrome is excluded from removal" -ForegroundColor Green
    Write-Host "  - External drives will be preserved" -ForegroundColor Green
    Write-Host "  - This script will not delete itself" -ForegroundColor Green
